...
// FOR Water level
Ultrasonic ultrasonic (8,6);  //trig- echo
float distance;
...
void loop(){
  ...
  // Update data on LCD every 1 second
  // Update data on Cloud every 115 second
  int var = 1;
  while (var < 10) {
  //Sensor measurements  
...
//  Serial.println(kirim);  
  ...
  delay(5000);// This is important, need longer time if you send longer data 
}
...
menu5:
          ...
          if (up == LOW) { delay(50); lcd.setCursor(9,0); lcd.print("      ");  tdsValue = tdsValue+1; }
          if (down == LOW) { delay(50); lcd.setCursor(9,0); lcd.print("      "); tdsValue = tdsValue-1; }
